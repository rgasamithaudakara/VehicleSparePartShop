...
export default class LoginSignin extends Component {
  ...
  handleClick = () => {
    ...
    if (this.props.title === 'SignUp') {
      let { fullname, email, password, verifyPassword } = this.inputText
      if (check.checked == true)
	  {
        email = email + '.lk'
		swal("user created!", "administrator will verify your profile and confirm for admission to onboard as a seller", "success");
	  }
	  else
	  {
		  swal("user created!", "user created successfully", "success");
	  }
